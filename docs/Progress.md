...
- 17 Sep 2026 — Real bugs caught by user testing: Patient Portal used top-nav instead of the sidebar pattern used everywhere else in the app (inconsistent), and the self-signup form was missing DOB/Gender/Address fields entirely (so they showed blank with no way to fill them in). Fixed: backend now accepts dob/gender/address at signup AND added `PATCH /api/patients/me` so patients can edit their own profile (allergies/chronic conditions/address/etc.) later — NOT name/MRN/CNIC, which stay staff-controlled for record integrity. Fix prompt sent to Antigravity for the sidebar conversion + missing form fields + a new "Edit Profile" feature.
- 17 Sep 2026 — Built `seed-demo-data.js`: seeds realistic Pakistani-context demo data (7 staff across all roles, 6 patients with full realistic details, appointments, consultations, a ward+admission, lab orders, pharmacy stock+dispense, bills in various payment states) directly into the user's real hospital via the live API, so every module has real content to test against instead of empty states.